...
(function () {
  ...
  var SECTIONS = [
    ...
    ["Productivity", [
      ...
      ["Pacing", "Productivity \u2192 Pacing: the school scheme of work, both personal curriculums and IT F201 on ONE week-indexed timeline. School week N is your week N \u2212 1, so the two are always matched on the week beginning and never on a bare number. The Week tab is a term ribbon (per-subject load, half term, mock weeks, the week you are in) over one week shown as three subject columns, each running what class covers \u2192 the alignment read \u2192 what your own plan covers. Every week is linkable: #/pacing/2026-12-14 reopens the week of the mocks."],
      ["Ticking off the plan", "Every row of your own plan has a tick beside it \u2014 on the Pacing week and on the Home card. Tick it when you have done that week's item (it is the plan's own bookkeeping, not a mastery claim: the topic page still owns that, and nothing is paid for a tick). An unticked row whose week has ended carries over into every later week, in a \u201cCarried over \u00b7 N behind\u201d band that names the week it came from and how many weeks late it is, until you tick it or press \u2192 here to move it into the week you are looking at. The row itself never changes weeks on its own. Class mocks, assessments and NEA milestones stand in the Countdowns; assignments due inside the week show under each subject; a row's dialog can set a reminder for the week's end; and the week rolling over with work left is one line in Notifications."],
      ["The alignment read", "The line between the two registers is the only thing on the page that is an argument rather than a list, and it is the reason the three plans had to become one store: for each specification point class is teaching this week, has your own plan already been through it, is it landing the same week, is it still ahead of you, or is it not in your plan at all? It is set arithmetic over linked spec points, measured from the FIRST week your plan reaches each one. A week whose class rows link to no spec point says so instead of drawing an empty verdict."],
      ...
      ["Editing the plan", "The plan was imported once and this app is the source of truth now, so it is fully editable. \u201c+ Add\u201d sits in each register; clicking any row opens it for editing with a searchable specification picker; the ribbon ends with a \u201c+ Week\u201d cell and each week has Edit week. Moving a week to a different beginning takes its rows with it, and deleting a week that still holds rows tells you how many it would take before you confirm. Delete always sits apart from Save and never confirms on Enter."],
      ["The braid", "The second Pacing tab draws the same evidence as topology instead of a sentence. Each subject gets a spine \u2014 the room's timeline, the clock nobody controls \u2014 with your own planned weeks ticked beneath it. Every specification point the two share becomes a BRANCH: it leaves your row and merges into the spine when you got there first, or leaves the spine and ends on your row when class got there first. Branches are packed into depths the way a commit graph packs lanes. \u25b3 on the spine marks points class teaches that your plan never schedules. Under the diagram, every branch is repeated as a button \u2014 that list is the keyboard and screen-reader route, and the only place with room to name the spec points the arcs can only encode as thickness."],
    ...
    ["The Collection Matrix", [
      ...
      ["Visual Novels vault", "VNDB fills the metadata (title, developer, cover, content tags as genres, length estimate); the tracking that matters is manual by design — VNDB has no structured route data, so the route list is yours to build. A VN's progress comes from ONE of four things you choose in the editor's “What counts”: routes cleared (the default when routes exist), chapters/parts completed (your own division of a VN's structure — how a kinetic novel like Higurashi counts, and it gets a “+1 ch” on the card), hours played measured against VNDB's crowd-sourced length (for a first playthrough of something with no clean structure — you can't know a percentage, but you know how long you've played; “+1 hr” on the card, and the length bucket stands in as a rough figure until VNDB has play-time votes), or a percentage you set. Left on Automatic, it counts routes, then chapters, then hours, then the percentage. Plus: a CG counter (numbers only, never artwork), your own content-warning tags, and the quote log — any kept line can be sent to the flashcard system's Personal deck, where it joins the normal SM-2 schedule without being forced into a subject."],
