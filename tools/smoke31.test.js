...
step("Status uses one identity hero and five non-duplicated instruments", async () => {
  ...
  if (!main.querySelector(".gov-seat-hero .id-command-mark")) throw new Error("unique Governor hero missing");
...
step("Recovery is one prescriptive dispatch, not a three-row route map", async () => {
  ...
  if (!/Core notes, quizzes, flashcards, and exam questions remain open/.test(rec.textContent)) throw new Error("access reassurance missing");
...
step("Gold Shop separates tools, simulations and cosmetics with contextual previews", async () => {
  ...
  if (!main.querySelector(".sp-seal-profile") || !main.querySelector(".sp-shelf") || !main.querySelector(".sp-shrine")) throw new Error("cosmetic context previews missing");
...
step("Avatar workshop uses the shared cropper and preserves seal/frame choices", async () => {
  ...
  await tick(40);
  const main = document.getElementById("main");
  if (!main.querySelector(".identity-stage") || !main.querySelector(".av-workshop-head")) throw new Error("avatar studio hierarchy missing");
...
step("Corner identity renders status as a speech bubble with compact actions", async () => {
  KOS.governor.setProfileText({ status: "Deep work until noon", about: "A small profile note." });
  KOS.refreshHUD();
  if (!document.querySelector("#hud .hud-status.profile-speech")) throw new Error("HUD status bubble missing");
  const pop = KOS.governor.openProfilePopover();
  if (!pop.querySelector(".pc-status.profile-speech")) throw new Error("popover status bubble missing");
...
step("Responsive and reduced-motion Governor rules are present", () => {
  ...
  if (!/\.heat-svg svg \{ width: auto !important; max-width: 100%/.test(css)) throw new Error("native heatmap geometry rule missing");
});
