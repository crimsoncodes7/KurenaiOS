...
step("Anime: no manual add, no Delete; the card carries a corner score and a one-line hover row", async () => {
  ...
  const btns = [...main().querySelectorAll("[data-ui~='vault.toolbar'] button")].map(b => b.textContent.trim());
  ...
  const corner = card.querySelector("[data-ui~='vault.score-corner']");
  assert(corner && corner.value === "8", "the score does not sit in the card corner");
  assert(card.querySelector("[data-ui~='vault.cover']") && corner.previousElementSibling === card.querySelector("[data-ui~='vault.cover']"), "the corner score must be a sibling of the cover, positioned by CSS");
  ...
  assert(modal && !modal.querySelector("[data-ui~='vault.delete-actions'] [data-intent~='danger']"), "the anime record offers Delete");
...
step("Books: the Digital lens lists AniList rows only and its primary is Find new; Physical keeps + Add to shelf", async () => {
  ...
  assert(/Find new/.test(main().querySelector("[data-ui~='vault.toolbar'] button[data-intent~='primary']").textContent), "digital primary must be Find new");
  main().querySelectorAll("[data-ui~='books.lens']")[1].click();
  await waitFor(() => /Shelf only/.test(main().textContent), 4000);
  assert(/Add to shelf/.test(main().querySelector("[data-ui~='vault.toolbar'] button[data-intent~='primary']").textContent), "physical primary must be + Add to shelf");
  ...
  modal.querySelector("[data-ui~='ui.form'] input[data-ui~='ui.quick-add']").value = "No volumes";
  modal.querySelector("[data-ui~='vault.save-actions'] button[data-intent~='primary']").click();
  await tick(30);
...
step("Seasonal: every status of the season (watching first), with the season's art and credit", async () => {
  ...
  assert(new RegExp(meta.credit).test(main().querySelector("[data-ui~='anime.season-credit']").textContent), "artist credit missing");
...
step("the overview: tabs in the page header, no streaks, no figures; the schedule and on-the-go cards", async () => {
  ...
  KOS.show("matrix");
  await waitFor(() => main().querySelector("[data-ui~='coll.now-card']"), 4000);
  assert(main().querySelector("[data-ui~='ui.page-head'] [data-ui~='ui.page-actions'] [data-ui~='coll.tabs'] [data-ui~='ui.tab']"), "the switcher must ride the page header's action slot");
  assert(!main().querySelector(".med-streaks") && !main().querySelector("[data-ui~='home.streak']"), "streak chips are back");
  assert(!main().querySelector("[data-ui~='ui.stat-strip']") && !main().querySelector("[data-ui~='chart.multi']"), "figures/charts are back on the overview");
  const card = main().querySelector("[data-ui~='coll.now-card']");
  assert(card.querySelector("[data-ui~='coll.now-plus']") && /\+1 ep/.test(card.querySelector("[data-ui~='coll.now-plus']").textContent), "the on-the-go card has no +1");
  card.querySelector("[data-ui~='coll.now-plus']").click();
