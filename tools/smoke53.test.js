...
step("the bell rides the global header with an unread badge and a five-row popover", async () => {
  ...
  const bell = document.querySelector("#topbar [data-ui~='shell.header-actions'] [data-ui~='notify.bell']");
  ...
  const badge = bell.querySelector("[data-ui~='notify.badge']");
  assert(badge && badge.textContent === "7" && badge.matches('[data-state~="is-on"]'), "badge wrong");
  bell.click();
  await tick(30);
  const panel = document.querySelector("[data-ui~='ui.menu-panel'][data-ui~='notify.panel']");
  assert(panel, "the popover did not open");
  assert(panel.querySelectorAll("[data-ui~='notify.row']").length === 5, "the popover must show the five most recent, got " + panel.querySelectorAll("[data-ui~='notify.row']").length);
  assert(/Row 0/.test(panel.querySelector("[data-ui~='notify.row']").textContent), "newest first");
  ...
  assert(!document.querySelector("[data-ui~='ui.menu-panel'][data-ui~='notify.panel']"), "the popover must close");
});
...
step("the favicon is wired and the seasonal hero honours a user picture from kv", async () => {
  ...
  KOS.show("seasonal");
  await waitFor(() => main().querySelector("[data-ui~='anime.season-picker']"), 3000);
  const sel = main().querySelector("[data-ui~='anime.season-picker'] [data-ui~='ui.status-select']");
  sel.value = "WINTER";
  sel.dispatchEvent(new window.Event("change", { bubbles: true }));
  await waitFor(() => (main().querySelector("[data-ui~='anime.season-art']") || {}).src === "https://x/mine.jpg", 3000);
  const art = main().querySelector("[data-ui~='anime.season-art']");
  assert(art.src === "https://x/mine.jpg" && art.matches('[data-state~="crop-media"]') && !art.getAttribute("srcset"), "the user's picture is not painted through the crop vars");
  assert(/your picture/.test(main().querySelector("[data-ui~='anime.season-credit']").textContent), "the credit must say it is yours");
